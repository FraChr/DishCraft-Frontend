...
import { trashIcon } from '@/Assets/svg icons/icons.ts'
...
const selectedOption = (name: string) => {
  ...
  if (props.multiple) selectMultiple(name, current)
  else {
...
const selectMultiple = (name: string, current: string[]) => {
  if (current.includes(name)) deselect(name, current)
  else {
...
<template>
  <div class="filter-wrapper">
    ...
    <div class="button-wrapper">
      <div>
        <button @click="clearAll" class="button-clear reset-button">
